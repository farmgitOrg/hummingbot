...
class CrossExchangeMarketMakingStrategy(StrategyPyBase):
    ...
    def init_params(self,
        ...
        self._maker_to_maker_filled_trades = {}
        # Holds maker order id to the maker filled trades that was hedged.
        self._taker_to_maker_filled_trades = {}
    ...
    @property
    def order_level_amount(self):
        return self._config_map.order_level_amount

    #order price, in quote asset
    @property
    def order_level_spread(self):
        return self._config_map.order_level_spread / Decimal("100")
        ...
        try:
            for order_level in range(0, self.order_levels):
                ...
                for maker_market, limit_order, order_id in self.active_maker_limit_orders:
                    if (self.maker_order_id_to_order_level[limit_order.client_order_id] != order_level):
                        continue  # skip
        ...
        if len(self._ongoing_hedging_table.keys()) > 0:
            for trade_id in self._ongoing_hedging_table.keys():
                if trade_id in self.hedge_buy_pending or trade_id in self.hedge_sell_pending:
                    ...
                    self.log_with_clock( logging.DEBUG, f"ready_for_new_trades: ongoing hedging & not pending trade_id: {trade_id}")
                    return False
        return True
    ...
    async def process_market_pair(self, timestamp: float, market_pair: MarketTradingPairTuple, active_orders: List, order_level: int):
        ...
        for active_order in active_orders:
            ...
            if timestamp > anti_hysteresis_timer:
                if not await self.check_if_price_has_drifted(market_pair, active_order, order_level):
    ...
    def did_fill_order(self, order_filled_event: OrderFilledEvent):
        ...
        if order_id in self._maker_to_taker_order_ids.keys():
            ...
            if order_id not in self._maker_to_maker_filled_trades.keys():
                self._maker_to_maker_filled_trades[order_id] = []
            if exchange_trade_id not in self._maker_to_maker_filled_trades[order_id]:
                ...
                self._maker_to_maker_filled_trades[order_id] += [exchange_trade_id]
                ...
                for maker_order_id in set(maker_order_ids_linked): # remove duplicates maker orders for currently completed taker order
                    ...
                    if len(active_taker_ids) == 0:
                        ...
                        if maker_order_id not in maker_order_ids:
                            ...
                            del self._maker_to_maker_filled_trades[maker_order_id]
                            del self.maker_order_id_to_order_level[maker_order_id]
                            del self.expected_hedging_prices[maker_order_id]
                        else:
                            self.log_with_clock(
                                logging.INFO,
                                f"did_complete_buy_order: maker_order_id: {maker_order_id} partial hedged by taker order_id: {order_id}"
                            )
                try:
                    ...
                    if  order_id in self._taker_to_maker_filled_trades.keys():
                        ...
                        self.log_with_clock(
                            logging.INFO,
                            f"did_complete_buy_order: taker order_id: {order_id} hedged maker_filled_trades: {maker_filled_trades}"
                        )
                        self.active_markets[0].trigger_event(event_tag=MarketEvent.OrderHedging, message=OrderHedgingEvent(maker_filled_trades, order_id));
                        del self._taker_to_maker_filled_trades[order_id]

                    self.hedge_sell_pending = []    #TODO: assume only 1 hedging for buy/sell
                except KeyError:
                ...
                for maker_order_id in set(maker_order_ids_linked): # remove duplicates maker orders for currently completed taker order
                    ...
                    if len(active_taker_ids) == 0:
                        ...
                        if maker_order_id not in maker_order_ids:
                            ...
                            self.log_with_clock(
                                logging.INFO,
                                f"did_complete_sell_order: maker_order_id: {maker_order_id} partial hedged by taker order_id: {order_id}"
                            )
                try:
                    # maker_exchange_trade_id = self._ongoing_hedging.inverse[order_id]
                    # del self._ongoing_hedging[maker_exchange_trade_id]
    ...
    async def check_if_price_has_drifted(self, market_pair: MakerTakerMarketPair, active_order: LimitOrder, order_level: int):
        """
        ...
        suggested_price = await self.get_market_making_price(market_pair, is_buy, order_quantity, order_level)
    ...
    async def check_and_hedge_orders(self, market_pair: MakerTakerMarketPair):
        ...
        :param market_pair: cross exchange market pair
        """
        order_level = -1; # it doesn't matter for taker order, we only care about maker order.
    ...
    def get_adjusted_limit_order_size(self, market_pair: MakerTakerMarketPair, order_level: int) -> Tuple[Decimal, Decimal]:
        ...
        if self._config_map.order_amount and self._config_map.order_amount > 0:
            ...
            order_size = base_order_size + order_level * self.order_level_amount
            return maker_market.quantize_order_amount(trading_pair, Decimal(order_size))
        else:
    ...
    async def get_market_making_size(self,
        ...
        size = self.get_adjusted_limit_order_size(market_pair, order_level)
    ...
    async def get_market_making_price(self,
        ...
        if is_bid:
            ...
            maker_price = taker_price / (1 + self.min_profitability + order_level * self.order_level_spread)
    ...
    async def check_and_create_new_orders(self,
        ...
        if not has_active_bid:
            bid_size = await self.get_market_making_size(market_pair, True, order_level)

            if bid_size > s_decimal_zero:
                bid_price = await self.get_market_making_price(market_pair, True, bid_size, order_level)
        ...
        if not has_active_ask:
            ask_size = await self.get_market_making_size(market_pair, False, order_level)

            if ask_size > s_decimal_zero:
                ask_price = await self.get_market_making_price(market_pair, False, ask_size, order_level)
    ...
    def place_order(self,
        ...
        if is_maker:
            ...
            self.log_with_clock(logging.INFO, f"place_order: taker_order_id: {order_id}, hedge maker_order_ids: {maker_order_ids}, maker_exchange_trade_ids: {maker_exchange_trade_ids}")
            ...
            self._taker_to_maker_filled_trades[order_id] = maker_exchange_trade_ids;
