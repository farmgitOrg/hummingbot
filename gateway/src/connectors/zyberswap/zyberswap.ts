...
import { ZyberswapConfig } from './zyberswap.config';
import routerAbi from './zyberswap_router_abi.json';
import { ContractInterface } from '@ethersproject/contracts';

import { Token, CurrencyAmount, Percent, TradeType } from '@uniswap/sdk-core';

import { Pair, Router, SwapParameters, Trade } from 'farm-zyberswap-sdk';
...
export class Zyberswap implements Uniswapish {
  private static _instances: { [name: string]: Zyberswap };
  ...
  private constructor(chain: string, network: string) {
    const config = ZyberswapConfig.config;
    if (chain === 'ethereum') {
    ...
    this._router = config.zyberswapRouterAddress(chain, network);
  }

  public static getInstance(chain: string, network: string): Zyberswap {
    if (Zyberswap._instances === undefined) {
      Zyberswap._instances = {};
    }
    if (!(chain + network in Zyberswap._instances)) {
      Zyberswap._instances[chain + network] = new Zyberswap(chain, network);
    }

    return Zyberswap._instances[chain + network];
  }
  ...
  getSlippagePercentage(): Percent {
    const allowedSlippage = ZyberswapConfig.config.allowedSlippage;
