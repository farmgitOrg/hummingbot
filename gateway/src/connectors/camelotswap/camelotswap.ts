...
import { CamelotswapConfig } from './camelotswap.config';
import routerAbi from './camelotswap_router_abi.json';
import { ContractInterface } from '@ethersproject/contracts';

import { Token, CurrencyAmount, Percent, TradeType } from '@uniswap/sdk-core';

import { Pair, Router, SwapParameters, Trade } from 'farm-camelot-sdk';

import CamelotPair from './camelot_pair_abi.json';
...
export class Camelotswap implements Uniswapish {
  private static _instances: { [name: string]: Camelotswap };
  ...
  private constructor(chain: string, network: string) {
    const config = CamelotswapConfig.config;
    if (chain === 'ethereum') {
    ...
    this._router = config.camelotswapRouterAddress(chain, network);
  }

  public static getInstance(chain: string, network: string): Camelotswap {
    if (Camelotswap._instances === undefined) {
      Camelotswap._instances = {};
    }
    if (!(chain + network in Camelotswap._instances)) {
      Camelotswap._instances[chain + network] = new Camelotswap(chain, network);
    }

    return Camelotswap._instances[chain + network];
  }
  ...
  getSlippagePercentage(): Percent {
    const allowedSlippage = CamelotswapConfig.config.allowedSlippage;
  ...
  async fetchData(baseToken: Token, quoteToken: Token): Promise<Pair> {
    ...
    const [reserves0, reserves1, _token0FeePercent, _token1FeePercent] =
    ...
    const pair = new Pair(
      ...
      parseInt(_token0FeePercent.toString()),
      parseInt(_token1FeePercent.toString())
    );
    return pair;
  }
